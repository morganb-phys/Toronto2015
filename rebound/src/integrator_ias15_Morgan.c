	...
	if( dt < 10. ){
		...
		for (int i=0;i<N-N_megno;i++){
		  ...
		  for (int j=0;j<i;j++){
		    ...
		    //fprintf(stderr,"The distance between %d and %d is %f. dt = %f\n",i,j,sqrt(r2),dt);
		    
		    //FILE * fp;
		    
		    //fp = fopen("data7.txt","a+");
		    //fprintf(fp, "%f\t%f\n",t,dt);
		...
		//if (mind != 10){
		fprintf(stderr, "Close encounter between %d and %d at time %f yr.\ndt = %e  Distance = %e\n", mini, minj,t,dt,mind);
		  
		//}
		
		if(mind < 0.01){

		  //fprintf(stderr, "Close encounter between %d and %d at time %e yr.  Distance = %e\n", mini, minj,t, mind);
		  particles[0].x = NAN;
		  particles[0].y = NAN;
		  particles[0].z = NAN;
